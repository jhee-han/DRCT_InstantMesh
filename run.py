...
parser.add_argument('--copy_dir', type=str, help='Directory to copy generated images to.') #InstantMesh/DRCT/datasets or InstantMesh/IPG/imgs
...
for idx, image_file in enumerate(input_files):
    ...
    images = rearrange(images, 'c (n h) (m w) -> (n m) c h w', n=3, m=2)        # (6, 3, 320, 320) #6장의 RGB 320 * 320 resolution images

    # Save each image separately(3*2의 이미지를 개별 6장의 이미지로 변환)
    to_pil = ToPILImage()
    for i, img_tensor in enumerate(images):
        img = to_pil(img_tensor)
        img_save_path = os.path.join(image_path, f'{name}_part{i+1}.png')
        img.save(img_save_path)
        print(f"Image part {i+1} saved to {img_save_path}")

        # 복사된 부분: 각 이미지를 사용자 지정 디렉토리에 복사
        if args.copy_dir:
            copy(img_save_path, os.path.join(args.copy_dir, f'{name}_part{i+1}.png'))
            print(f"Image part {i+1} copied to {os.path.join(args.copy_dir, f'{name}_part{i+1}.png')}")
